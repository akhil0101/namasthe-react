...
import { MENU_ITEM_TYPE_KEY, RESTAURANT_TYPE_KEY } from "../service/constants.js"
...
const RestaurantMenu = () => {
    ...
    useEffect(() => {
        if (data) {
            const restaurantData = data?.cards?.map(x => x.card)?.
    ...
    const categories = data?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards.filter((c)=>{
